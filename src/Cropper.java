...
public class Cropper implements Runnable {
    ...
    private void crop(Pair<File, File> pair) {
        List<String> txt = fileIO.readTxt(pair.getKey());
        BufferedImage img = fileIO.readImg(pair.getValue());
        ...
        if (txt != null && img != null) {
            ...
            if (regions.size() > 0) {
                ...
                for (int i = 0; i < regions.size(); i++) {
                    ...
                    String basePath = pair.getKey().getParent() + "\\";
                    String txtPath = basePath + fileIO.getFileNameWithoutExtension(pair.getKey()) + "_" + i + "." + fileIO.getFileExtension(pair.getKey());
                    String imgPath = basePath + fileIO.getFileNameWithoutExtension(pair.getValue()) + "_" + i + "." + fileIO.getFileExtension(pair.getValue());
